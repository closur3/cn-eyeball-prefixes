...
func TestClassifyPrefixRequiresUniformPositiveCoverage(t *testing.T) {
	...
	segments := []apnic6.Segment{{Lo: prefix.Addr(), Hi: iputil.LastAddress(prefix), Record: mobile}}
	...
	segments = []apnic6.Segment{
		...
		{Lo: middle, Hi: iputil.LastAddress(prefix), Record: fixed},
	}
...
func TestNonAdmittedMoreSpecificRegistrationCreatesAdmissionGap(t *testing.T) {
	...
	segments := []apnic6.Segment{
		...
		{Lo: middle, Hi: iputil.LastAddress(prefix), Record: nonAccess},
	}
