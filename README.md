...
- `data/experiments/bgp-prefix-admission/*-hybrid.txt` 是低风险混合对照：完整保留当前 `dev`，只利用三网 BGP 宣告修复同运营商父级范围内由三网内部最具体登记冲突造成的孔洞。它不会因 BGP 单元边界删除现行地址，也不会放回缺少三网 APNIC 父级的独立资源；对应 `*-hybrid-added.txt` 与 `*-hybrid-removed.txt` 必须证明实际变化符合这一边界。
