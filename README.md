...
| 明确的骨干基础设施登记 | IPv4 的明确 APNIC 排除向子登记继承；未知子登记不能覆盖，只有更具体且明确的终端接入证据可解除可覆盖排除 |
| 明确位于目标地理范围之外的地址登记 | IPv4 硬排除，不得由更具体登记解除 |
| 专用精品骨干 | IPv4 按 Origin ASN 排除；IPv6 的已确认接入地址段允许由其承载 |
...
骨干基础设施没有许可清晰、覆盖完整且持续更新的公开成品上游。生成器结合 APNIC 登记层级与当前 BGP Origin 判定：明确的骨干基础设施证据形成可继承的运营侧排除；用途不明的更具体登记不得覆盖父级判定；只有同时具备接入技术和终端地址用途证据的更具体登记，才能解除可覆盖排除。明确位于目标地理范围之外的登记不可解除。单独的部门名称或 ASN 描述不作为骨干排除依据。
...
IPv6 的详细准入来源、Origin、拒绝原因和输入摘要作为更新工作流的审计产物上传并保留 30 天，不写入精简的公开 manifest。
...
6. 生成公开 manifest，并上传来源哈希、详细证据和变更日志等审计产物；
7. 仅在公开列表确有变化时提交更新。数据完整性校验失败会保留上一版正式列表，变化幅度告警不会阻止发布。
...
本项目自有代码和人工维护内容按 [MIT License](LICENSE) 发布。

第三方数据的来源、许可证和使用说明见 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)，相关数据仍遵循各自上游条款。
