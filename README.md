...
本仓库提供**自动维护的中国大陆三网终端用户互联网接入 IPv4 候选列表**，明确用于 ACL 白名单。列表仅收录中国电信、中国移动和中国联通面向真实终端用户提供公网互联网接入服务的地址，并显式排除具有可靠强证据的非终端接入网络；不承诺穷尽所有代宣告、托管和混合用途地址。终端用户包括家庭、移动、企业及机构用户。

输出采用 best-effort 口径，不是中国三网终端用户互联网接入地址的穷尽清单或绝对真值。构建器按运营商归属筛选，并以 ASN 声明用途、少量明确的 ASN 政策例外、云厂商 CIDR、APNIC 最具体前缀登记和当前路由证据执行自动清洗：明确标为 IDC、CDN、云计算、数据中心、托管、机房、传输骨干、非公网专网、IoT/M2M、网络安全专网或内部 OA 的网络会自动排除；登记给可精确关联当前活跃独立 ASN 主体、但当前由三网 ASN 宣告的 portable/non-portable 前缀也会排除。企业或机构的公网互联网接入地址属于收录范围；仅有企业名称、non-portable 状态或专线接入属性不会单独触发排除。证据不足的地址保留在候选列表中。
...
flowchart TD
    ...
    Q --> Y
    W --> Y
    U["RIPE RISWhois：多观测点 origin 集合"] --> V["保守 MOAS：三网 origin + 强非终端接入备用 origin"]
    T --> Y
    Y --> V
    C --> V
    V --> J["最终三网地址池"]
...
- 运营商匹配规则统一维护在 `config/operators.json`。`include_asns` 补充名称无法识别的三网 ASN；`exclude_description_rules` 自动识别用途明确的非终端接入 ASN；`exclude_asns` 只处理有明确证据、无法由通用描述规则可靠表达的例外。manifest 会区分 `description_rule` 和 `explicit_policy` 两类排除来源。
...
- [RIPE RISWhois](https://ris.ripe.net/docs/ris-whois/) 提供多个 RIS 采集点汇总的当前前缀/origin 与可见 peer 数。这里只处理三网候选范围，并按最具体 BGP 前缀判断：当前三网 origin 和备用 origin 均须至少被 10 个 peer、且达到该前缀最高可见度的 5%；备用 origin 的当前 IPtoASN 描述还必须命中同一套强非终端接入规则，才会剔除。普通 MOAS、低可见度 origin、描述未知或证据含糊的情况只计入审计统计并继续保留。
